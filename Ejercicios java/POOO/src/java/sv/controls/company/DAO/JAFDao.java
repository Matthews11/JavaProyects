...
public class JAFDao {
    ...
    private static String INSERT_USUARIOS_SQL = "insert into Casos (DescripcionA,IdDepartamento,IdEstatoCaso , IdEmpleado,IdJefeFuncional) values "
            + "(?,?,?,?,?)";

    private static String aceptar = "update Casos set DescripcionJ=?,IdProgramador=?,FechaJD=?,IdEstatoCaso=?"
            + " where IdCaso=?";

    private static String rechazar = "update Casos set RazonRechazo=?,IdEstatoCaso=?   where IdCaso=?";
    ...
    public void aceptar(CasosBeans CB) throws SQLException {
        ...
        try (Connection conn = getConnection();
            ...
            statement.setInt(5, CB.getIdCaso());
            statement.executeUpdate();
    ...
    public void rechazar(CasosBeans CB) throws SQLException {
        ...
        try (Connection conn = getConnection();
            ...
            statement.setInt(3, CB.getIdCaso());
